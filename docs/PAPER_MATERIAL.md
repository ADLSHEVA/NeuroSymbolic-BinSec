...
| | Precision | Recall | F1 |
|---|---|---|---|
| 阶段1 标准(名字/结构启发式) | 0.794 | 0.711 | 0.750 |
| **阶段2 +GNN 类型驱动** | **0.833** | **0.921** | **0.875** |

**两个关键结论**:
1. **指标如实下降**:真实基准上从 6 程序的 100% 降到 ~75% 基线——这是诚实、可外推的科研数据(toy 100% 不能外推)。
2. **GNN 的量化价值**:GNN 类型驱动把召回 **0.711 → 0.921**(+21 点),救回 8 个 **CWE121 栈溢出**——这些是"缓冲区索引越界写、无 libc sink 名"的用例,名字/结构启发式**全部漏掉**,只有 GNN 恢复的类型能补。这把"GNN 有用"从定性变成了**在真实数据上量化**。
...
4. **LLM 延迟**：裁决 ~30–40s/路径，是吞吐瓶颈。
5. **不 sound/complete**：见威胁模型。
...
A: LLM **不单独决策**。它在两类"事实"之上裁决：① 符号执行的硬证据(如 strncpy 的 size 约束、gets 无界)；② 污点路径的软阈值分数。LLM 做的是"结合证据解释并打分"，且我们记录其 CoT 便于审计。证据缺失时倾向保守。
